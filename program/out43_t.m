...
res2.names = {"Δp_гк";
    ...
    "Δpm_φ";
    "Δp_ПГ"}
res2.units(1:8,1) = "кПа";

% Преобразуем res.names и res.units в вертикальные массивы ячеек (если они ещё не такие)
if isstring(res2.names)
    namesCell = cellstr(res2.names(:));
elseif iscell(res2.names)
    namesCell = res2.names(:);
else
    error('res.names должен быть строковым массивом или массивом ячеек.');
end

if isstring(res2.units)
    unitsCell = cellstr(res2.units(:));
elseif iscell(res2.units)
    unitsCell = res2.units(:);
else
...
fileName = 'HydraulicResults.xlsx';
